...
- **Acceptance test:** the `factor_v2` adapter replayed over the 90 phase-2 games must reproduce the phase-2 diagnostics. Status (2026-06-11): **PASSED** — `code/evaluation/offline/reproduce_phase2.py` reproduces every documented figure (893 approvals; 598/269/26 = 67/30/3 buckets; 0.555/0.507/41% belief stats; 3.4× rejection ratio) from the committed artifacts (server JSONs + `CSV_(*)` belief traces + the real GoodPolicy code). The earlier caveat about lost `DEBUG_*.log` files applied only to the Mac checkout: the Windows machine that actually ran the grid still has all 540 logs, the recomputed risks were verified bit-exact against them (`--cross-check`), and their `POLICY_DECISION` streams are now committed as `policy_decisions_*.jsonl` per run (`--extract-decisions`). Recovered methodology notes: the doc's "belief" = the policy's behavior-adjusted `party_risk`; "threshold" bucket = non-hammer approvals with a true-evil member at risk ≥ 0.55 (the quest-5 base threshold); "forced" = approvals at `failed_party_votes ≥ 3`.
